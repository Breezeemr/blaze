(ns dromon.core
  (:require [dromon.system :as system]
...
(defn init! [config]
  (try
    ...
    (catch Exception e
      (log/error
        (cond->
          (str "Error while initializing Dromon `" (or (ex-message e) "unknown")
               "`")
          (ex-cause e)
