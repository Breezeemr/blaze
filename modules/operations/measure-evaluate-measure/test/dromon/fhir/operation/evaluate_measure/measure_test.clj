(ns dromon.fhir.operation.evaluate-measure.measure-test
  (:require
    [dromon.bundle :as bundle]
    [dromon.cql-translator :as cql-translator]
    [dromon.datomic.test-util :as datomic-test-util]
    [dromon.datomic.value :as value]
    [dromon.elm.compiler :as compiler]
    [dromon.fhir.operation.evaluate-measure.cql :as cql]
    [dromon.fhir.operation.evaluate-measure.measure :refer [evaluate-measure]]
    [dromon.handler.fhir.util :as fhir-util]
    [dromon.terminology-service.extern :as ts]
...
(defn- slurp-resource [name]
  (let [cl (.getContextClassLoader (Thread/currentThread))
        name (str "dromon/fhir/operation/evaluate_measure/" name)
