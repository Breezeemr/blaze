(ns dromon.rest-api
  (:require [buddy.auth.middleware :refer [wrap-authentication]]
            ...
            [dromon.bundle :as bundle]
            [dromon.middleware.fhir.metrics :as metrics]
            [dromon.module :refer [reg-collector]]
            [dromon.rest-api.middleware.auth-guard :refer [wrap-auth-guard]]
            [dromon.rest-api.middleware.json :as json :refer [wrap-json]]
            dromon.rest-api.spec
            dromon.spec
...
(defn resolve-pattern
  ...
  (or
    (some
      #(when (= name (:dromon.rest-api.resource-pattern/type %)) %)
      resource-patterns)
    (some
      #(when (= :default (:dromon.rest-api.resource-pattern/type %)) %)
      resource-patterns)))
...
(defn resource-route
  ...
  (when-let
    [{:dromon.rest-api.resource-pattern/keys [interactions]}
...
(defn router
  ...
  (non-conflict-detecting-router
    (-> [""
         {:dromon/base-url base-url
          :dromon/context-path context-path
          :middleware
          (cond-> []
        ...
        (into
          (mapcat
            (fn [{:dromon.rest-api.operation/keys
                  [code system-handler]}]
              (when system-handler
                [(str "/$" code)
                 {:middleware
...
(defn- capability-resource
  ...
  (when-let
    ...
    (let [operations
          (filter
            #(some #{name} (:dromon.rest-api.operation/resource-types %))
            operations)]
      (cond->
        {:type name
         :interaction
         (reduce
           (fn [res code]
             (if-let
               [{:dromon.rest-api.interaction/keys [doc]} (get interactions code)]
               (conj
                 res
                 (cond->
  ...
  (let [capability-statement
        {:resourceType "CapabilityStatement"
         ...
         {:name "Dromon"
          :version version}
         :implementation
         {:description (str "Dromon running at " base-url context-path)
...
(defmethod ig/pre-init-spec :dromon/rest-api [_]
  (s/keys
    :req-un
    [:dromon/base-url
     ::version
...
(defmethod ig/init-key :dromon/rest-api
