(ns dromon.interaction.transaction-test
  ...
  (:require
    ...
    [dromon.interaction.transaction :refer [handler]]
      ...
      (let [{:keys [status body]}
            @((handler ::transaction-executor ::conn ::term-service executor)
              {::reitit/router ::router
               ::reitit/match {:data {:dromon/context-path ""}}
               :body
               {"resourceType" "Bundle"
                "type" "transaction"
                "entry" entries}})]
  ...
  (testing "On updated resource in transaction"
    (let [resource
          ...
          [{"resource"
            ...
            {"method" "PUT"
             "url" "Patient/0"}
            :dromon/old-resource ::old-patient}]]
