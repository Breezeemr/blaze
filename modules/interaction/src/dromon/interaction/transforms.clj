...
;;NOTE this seems over done. I have to pull the key based on the value. It's usually the
;;reverse which is cleaner.
(defn expression-key->old-dromon-key
...
(defn ->expression
  ...
  (reduce
    (fn [coll exp-key]
      (if-let [old-key (expression-key->old-dromon-key exp-key mapping)]
        (conj coll old-key)
        (conj coll exp-key)))
    []
    expression))
...
(comment
  ...
  (def mapping
    {:type                                                  "MedicationPrescription"
     ...
     :fhir.Medication/code$cr                               {:key   :fhir.MedicationRequest/code
                                                             :value dromon.fhir.transforms/codeable-concept}
  ...
  (expression-key->old-dromon-key :fhir.MedicationRequest/subject mapping)
  ;; => :fhir.MedicationPrescription/patient

  (expression-key->old-dromon-key :foobar mapping)
  ;; => nil
