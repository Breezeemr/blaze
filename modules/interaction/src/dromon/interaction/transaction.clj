(ns dromon.interaction.transaction
...
(defn- validate-entry
  ...
  (let [[type id] (some-> url bundle/match-url)]
    (cond
      ...
      :else
      (assoc entry :dromon/type type :dromon/id id))))
  ...
  (cond
    ...
    (let [{type "resourceType" id "id"} resource]
      (if-let [resource (util/resource db type id)]
        (assoc entry :dromon/old-resource resource)
        entry))
...
(defmethod build-response-entry "PUT"
  [{:keys [router return-preference]}
   {{type "resourceType" id "id"} "resource" :dromon/keys [old-resource]}
   {db :db-after}]
  (let [last-modified (util/tx-instant (util/basis-transaction db))
        vid (str (d/basis-t db))]
    (cond->
...
(defn- process-batch-entry
  [{:keys [handler] :dromon/keys [context-path]}
          ...
          (md/chain'
            (let [context
                  {:router router
                   ...
                   :dromon/context-path (-> match :data :dromon/context-path)
...
(defmethod ig/init-key :dromon.interaction.transaction/handler
