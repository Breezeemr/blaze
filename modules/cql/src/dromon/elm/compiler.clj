(ns dromon.elm.compiler
  ...
  (:require [clojure.spec.alpha :as s]
            ...
            [dromon.datomic.quantity :as fhir-quantity]
            [dromon.datomic.util :as db]
            [dromon.elm.aggregates :as aggregates]
            dromon.elm.boolean
            [dromon.elm.code :as code]
            [dromon.elm.compiler.function :as function]
            [dromon.elm.compiler.property :as property]
            [dromon.elm.compiler.protocols :refer [-eval -hash Expression]]
            [dromon.elm.compiler.query :as query]
            [dromon.elm.compiler.retrieve :as retrieve]
            [dromon.elm.data-provider :as data-provider]
            [dromon.elm.date-time :as date-time :refer [local-time temporal?]]
            [dromon.elm.decimal :as decimal]
            [dromon.elm.deps-infer :refer [infer-library-deps]]
            [dromon.elm.equiv-relationships :refer [find-equiv-rels-library]]
            dromon.elm.integer
            [dromon.elm.interval :refer [interval]]
            dromon.elm.list
            dromon.elm.nil
            [dromon.elm.normalizer :refer [normalize-library]]
            [dromon.elm.protocols :as p]
            [dromon.elm.quantity :refer [quantity quantity?]]
            dromon.elm.spec
            [dromon.elm.string :as string]
            [dromon.elm.type-infer :refer [infer-library-types]]
            [dromon.elm.util :as elm-util])
  (:import [java.time LocalDate LocalDateTime OffsetDateTime Year YearMonth ZoneOffset]
...
(defn static? [x]
  (not (instance? dromon.elm.compiler.protocols.Expression x)))
